...
int main(int argc, char **argv)
{
    ...
    vector<cv::KeyPoint> imKeypts;
    cv::Mat imDescs;

    cv::FileStorage fskeypts("keypointsLeft_r.yml", cv::FileStorage::READ);
    cv::FileStorage fsdescs("descriptorsLeft_r.yml", cv::FileStorage::READ);
    cv::FileNode kptFileNode, descFileNode;
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        for(int ni=0; ni<nImages[seq]; ni++, proccIm++)
        {
            ...
            ptr = strtok((char*)(vstrImageFilenames[seq][ni]).c_str(), "/"); // use strtok() function to separate string using comma (,) delimiter.  
            ...
            kptFileNode = fskeypts["img"+filename];
            descFileNode = fsdescs["img"+filename];

            read(kptFileNode, imKeypts);
            read(descFileNode, imDescs);
            ...
            SLAM.TrackMonocular(im,imKeypts,imDescs,tframe);
