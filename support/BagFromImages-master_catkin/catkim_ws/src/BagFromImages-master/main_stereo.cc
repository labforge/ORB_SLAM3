...
int main(int argc, char **argv)
{
    ...
    if(argc!=8)
    {
        cerr << "Usage: rosrun BagFromImages main_stereo <path to image directory> <image extension .ext> <frequency> <Left keypts & Descriptors> <Right keypts & Descriptors> <path to output bag>" << endl;
        return 0;
    }

    std::string left_dir = argv[1]; 
    std::string right_dir = argv[2]; 
    std::string file_ext = argv[3];
    std::string freq_arg = argv[4];
    std::string left_kptsndescs_dir = argv[5]; 
    std::string right_kptsndescs_dir = argv[6];
    std::string out_bag = argv[7];
