...
public:
    ...
    Frame(const cv::Mat &imGray, const vector<cv::KeyPoint> &imKeypts, const cv::OutputArray imDescs, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, GeometricCamera* pCamera, cv::Mat &distCoef, const float &bf, const float &thDepth, Frame* pPrevF = static_cast<Frame*>(NULL), const IMU::Calib &ImuCalib = IMU::Calib());
    ...
    void ExtractORB_r(int flag, const cv::Mat &im, const vector<cv::KeyPoint> &imKeypts, const cv::OutputArray imDescs, const int x0, const int x1);
